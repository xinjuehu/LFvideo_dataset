...
def genEPI(Frames_num_arg, Matrix_frames, EPI_frames, Col_num):
    ...
    for i in range(Frames_num):
        ...
        for w in range(w_5):
            for c in range(col):
                EPI_img_ver = np.zeros((row*5, height, channel))
                EPI_img_ver[:,:,0] = np.transpose(img_matrix_R[:, c, :, w*5:(w+1)*5], axes=[0, 2, 1]).reshape((row*5, height))
                EPI_img_ver[:,:,1] = np.transpose(img_matrix_G[:, c, :, w*5:(w+1)*5], axes=[0, 2, 1]).reshape((row*5, height))
                EPI_img_ver[:,:,2] = np.transpose(img_matrix_B[:, c, :, w*5:(w+1)*5], axes=[0, 2, 1]).reshape((row*5, height))
                EPI_img_file = EPI_frame_name + '%02d_%04d'%(c, w*5) + '.jpg'
                cv2.imwrite(EPI_img_file, EPI_img_ver)
