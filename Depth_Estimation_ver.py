...
def Evaluate_EPI(EPI_file, Mask_Roi):
    EPI_dir = '.\\EPI_frames_ver\\1\\'
    Depth_dir = '.\\Depth_frames_ver\\'
    ...
    # block_depth = block_depth.astype('uint8')
...
def Match_block(block_id, BLOCK_TABLE, IMG_SLICE, IMG_IN):
    ...
    # search_area_end = block_table[block_id,1] + search_area_ratio*(block_table[block_id,1] - block_table[block_id,0] + 1)
    # search_area_sta = block_table[block_id,0] - search_area_ratio*(block_table[block_id,1] - block_table[block_id,0] + 1)
    search_area_end = block_table[block_id,1] + 300
    search_area_sta = block_table[block_id,0] - 300
    # if search_area_sta <0:
    #     search_area_sta = 0
    # if search_area_end > img_in_len:
    #     search_area_end = img_in_len
    ...
    for line_indx in range(10):
        # search_sample = img_in[line_indx*2:(line_indx+1)*2, search_area_sta:search_area_end, :]
...
if __name__ == "__main__":
    ...
    blurred = cv2.GaussianBlur(central_view,(11,11),0)
    gaussImg = cv2.Canny(blurred, 10,50)
    for i in range(384):
        EPI_name = '04_' + '%04d'%(i*5)
        mask_roi = gaussImg[:, i*5:(i+1)*5].transpose([1,0])
